...
public class EntitlementOutboxPublisher {
    ...
    public void publishPendingBatch() {
        ...
        List<OutboxEventRecord> pending = outboxEventRepository.claimPending(
        ...
        for (OutboxEventRecord record : pending) {
            try {
                EntitlementEventPayload payload = parsePayload(record);
                EntitlementEvent event = buildEvent(record, payload);
                Headers headers = buildHeaders(record, payload);
                PublishAck ack = publishWithAck(headers, event);
                if (ack == null) {
                    throw new IllegalStateException("puback is missing");
                }
                int updated = outboxEventRepository.markPublished(record.eventId(), lockedBy, now);
                if (updated == 0) {
    ...
    private EntitlementEvent buildEvent(OutboxEventRecord record, EntitlementEventPayload payload) {
        EntitlementEvent.EventType eventType = mapEventType(record.eventType());
    ...
    private PublishAck publishWithAck(Headers headers, EntitlementEvent event) throws IOException, JetStreamApiException {
    ...
    private void handleFailure(OutboxEventRecord record, Exception ex, Instant now, String lockedBy) {
        boolean nonRetryable = ex instanceof OutboxPayloadParseException;
        int nextAttempt = nonRetryable ? properties.maxAttempts() : record.attemptCount() + 1;
        boolean failed = nonRetryable || nextAttempt >= properties.maxAttempts();
        Instant nextRetryAt = failed ? null : now.plusSeconds(computeBackoffSeconds(nextAttempt));
        int updated = outboxEventRepository.markFailure(
                record.eventId(),
                lockedBy,
                nextAttempt,
        ...
        if (updated == 0) {
            logger.warn("outbox retry skipped because lock was lost eventId={} attempt={}",
                    record.eventId(),
                    nextAttempt);
        }
        if (failed) {
            if (nonRetryable) {
                ...
                logger.error("outbox payload parse failed and moved to FAILED eventId={}", record.eventId(), ex);
            } else {
                logger.warn("outbox publish moved to FAILED eventId={}", record.eventId(), ex);
            }
        } else {
            logger.warn("outbox publish retry scheduled eventId={} attempt={}",
                    record.eventId(),
                    nextAttempt,
                    ex);
        }
