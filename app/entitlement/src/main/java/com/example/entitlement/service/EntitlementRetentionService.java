...
public class EntitlementRetentionService {
    ...
    public void cleanup() {
        ...
        int deletedIdempotency = idempotencyKeyRepository.deleteExpired(now);
        // outbox は publish 済みのみ対象にするため、published_at の TTL を使う。
        Instant outboxThreshold = now.minus(Duration.ofHours(outboxProperties.publishedTtlHours()));
        int deletedOutbox = outboxEventRepository.deletePublishedOlderThan(outboxThreshold);
        logger.info("entitlement retention cleanup deleted idempotencyKeys={} outboxEvents={} outboxThreshold={} idempotencyThreshold={}",
                deletedIdempotency,
                deletedOutbox,
                outboxThreshold,
                now);
