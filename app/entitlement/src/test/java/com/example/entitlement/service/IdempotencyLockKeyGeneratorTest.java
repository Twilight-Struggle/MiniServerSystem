...
class IdempotencyLockKeyGeneratorTest {
    ...
    void generateIsDeterministic() {
        long first = generator.generate(IDEM_KEY);
        long second = generator.generate(IDEM_KEY);
    ...
    void generateChangesWhenKeyChanges() {
        long first = generator.generate(IDEM_KEY);
        long second = generator.generate(IDEM_KEY_OTHER);
    ...
    void generateHandlesEmptyKeyDeterministically() {
        long first = generator.generate(IDEM_KEY_EMPTY);
        long second = generator.generate(IDEM_KEY_EMPTY);
