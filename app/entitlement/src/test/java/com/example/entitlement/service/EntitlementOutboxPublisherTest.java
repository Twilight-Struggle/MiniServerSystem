...
import com.example.entitlement.model.EntitlementEventPayload;
...
class EntitlementOutboxPublisherTest {
        ...
        private static final EntitlementOutboxProperties PROPERTIES = new EntitlementOutboxProperties(
                        true,
                        1000,
                        50,
                        10,
                        ...
                        30,
                        24);
        private static final EntitlementNatsProperties NATS_PROPERTIES = new EntitlementNatsProperties(
                        "entitlement.events",
                        "entitlement-events",
        ...
        void setUp() {
                ...
                publisher = new EntitlementOutboxPublisher(
                                jetStream,
        ...
        void publishPendingBatchMovesToFailedWhenPayloadParseFails() {
                ...
                verify(outboxEventRepository, never()).markPublished(any(UUID.class), anyString(), any(Instant.class));

                // FAILED への即時遷移を検証する
                ArgumentCaptor<String> lockedByCaptor = ArgumentCaptor.forClass(String.class);
                ...
                verify(outboxEventRepository).markFailure(
                                eq(record.eventId()),
        ...
        @Test
        void publishPendingBatchMarksPublishedWhenPubAckIsReceived() throws Exception {
                UUID eventId = UUID.randomUUID();
                OutboxEventRecord record = buildValidRecord(eventId);
                ...
                when(outboxEventRepository.markPublished(eq(eventId), anyString(), eq(FIXED_NOW))).thenReturn(1);
                ...
                verify(outboxEventRepository, never())
                                .markFailure(any(UUID.class), anyString(), anyInt(), any(OutboxStatus.class), any(Instant.class),
                                                anyString());
        }

        @Test
        void publishPendingBatchMarksFailureWhenPubAckFails() throws Exception {
                UUID eventId = UUID.randomUUID();
                OutboxEventRecord record = buildValidRecord(eventId);
        ...
        @Test
        void publishPendingBatchMarksFailureWhenPubAckIsNull() throws Exception {
                UUID eventId = UUID.randomUUID();
                OutboxEventRecord record = buildValidRecord(eventId);
        ...
        @Test
        void publishPendingBatchMarksFailureWhenJetStreamApiExceptionOccurs() throws Exception {
                UUID eventId = UUID.randomUUID();
                OutboxEventRecord record = buildValidRecord(eventId);
        ...
        @Test
        void publishPendingBatchTruncatesErrorMessage() throws Exception {
                UUID eventId = UUID.randomUUID();
                OutboxEventRecord record = buildValidRecord(eventId);
                String longMessage = "x".repeat(PROPERTIES.errorMessageMaxLength() + 5);
                ...
                assertThat(errorCaptor.getValue()).isEqualTo(longMessage.substring(0, PROPERTIES.errorMessageMaxLength()));
        }

        @Test
        void publishPendingBatchContinuesWhenMarkPublishedReturnsZero() throws Exception {
                UUID firstId = UUID.randomUUID();
                UUID secondId = UUID.randomUUID();
                OutboxEventRecord first = buildValidRecord(firstId);
                OutboxEventRecord second = buildValidRecord(secondId);
                ...
                verify(jetStream, times(2)).publish(eq(NATS_PROPERTIES.subject()), any(Headers.class), any(byte[].class));
                verify(outboxEventRepository, times(2)).markPublished(any(UUID.class), anyString(), eq(FIXED_NOW));
                verify(outboxEventRepository, never())
                                .markFailure(any(UUID.class), anyString(), anyInt(), any(OutboxStatus.class), any(Instant.class),
                                                anyString());
        }

        private OutboxEventRecord buildValidRecord(UUID eventId) throws Exception {
