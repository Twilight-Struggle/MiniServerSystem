...
class IdempotencyKeyRepositoryTest extends AbstractPostgresContainerTest {
  ...
  void insertAndFindReturnsRecord() {
    ...
    final IdempotencyRecord record =
        new IdempotencyRecord(IDEM_KEY, REQUEST_HASH, RESPONSE_CODE, RESPONSE_BODY, now.plus(TTL));

    idempotencyKeyRepository.insert(record);

    final Optional<IdempotencyRecord> stored = idempotencyKeyRepository.findByKey(IDEM_KEY);
  ...
  void upsertIfExpiredReturnsZeroWhenNotExpired() {
    ...
    final IdempotencyRecord existing =
        new IdempotencyRecord(IDEM_KEY, REQUEST_HASH, RESPONSE_CODE, RESPONSE_BODY, expiresAt);
    idempotencyKeyRepository.insert(existing);

    final IdempotencyRecord desired =
        new IdempotencyRecord(
            IDEM_KEY, REQUEST_HASH_ALT, RESPONSE_CODE_ALT, RESPONSE_BODY_ALT, expiresAt.plus(TTL));

    final int updated = idempotencyKeyRepository.upsertIfExpired(desired);
  ...
  void upsertIfExpiredUpdatesWhenExpired() {
    ...
    final Instant now = Instant.now();
    final Instant expiredAt = now.minus(TTL);
    final IdempotencyRecord expired =
        new IdempotencyRecord(IDEM_KEY, REQUEST_HASH, RESPONSE_CODE, RESPONSE_BODY, expiredAt);
    idempotencyKeyRepository.insert(expired);

    final Instant newExpiresAt = now.plus(TTL);
    final IdempotencyRecord desired =
        new IdempotencyRecord(
            IDEM_KEY, REQUEST_HASH_ALT, RESPONSE_CODE_ALT, RESPONSE_BODY_ALT, newExpiresAt);
  ...
  void deleteExpiredRemovesOnlyExpired() {
    ...
    final IdempotencyRecord expired =
        new IdempotencyRecord(
            IDEM_KEY, REQUEST_HASH, RESPONSE_CODE, RESPONSE_BODY, now.minusSeconds(1));
    final IdempotencyRecord active =
        new IdempotencyRecord(
            IDEM_KEY_ACTIVE, REQUEST_HASH, RESPONSE_CODE, RESPONSE_BODY, now.plus(TTL));
    idempotencyKeyRepository.insert(expired);
    idempotencyKeyRepository.insert(active);

    final int deleted = idempotencyKeyRepository.deleteExpired(now);

    final MapSqlParameterSource expiredParams =
        new MapSqlParameterSource().addValue("idemKey", IDEM_KEY);
    final Integer expiredCount =
        jdbcTemplate.queryForObject(
            "SELECT count(*) FROM idempotency_keys WHERE idem_key = :idemKey",
            expiredParams,
            Integer.class);
    final MapSqlParameterSource activeParams =
        new MapSqlParameterSource().addValue("idemKey", IDEM_KEY_ACTIVE);
  ...
  private void assertJsonEqual(String expectedJson, String actualJson) {
    try {
      final JsonNode expected = OBJECT_MAPPER.readTree(expectedJson);
      final JsonNode actual = OBJECT_MAPPER.readTree(actualJson);
