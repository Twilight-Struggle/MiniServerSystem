...
class NotificationRetentionServiceTest extends AbstractPostgresContainerTest {
  ...
  @Autowired private NotificationRetentionService retentionService;

  @Autowired private NotificationRetentionProperties retentionProperties;
  ...
  void cleanupRemovesOnlySentAndFailed() {
    ...
    final int retentionDays = retentionProperties.retentionDays();
    final Instant threshold = now.minus(Duration.ofDays(retentionDays));
    final Instant old = now.minus(Duration.ofDays(retentionDays + 1));
    final Instant recent = now.minus(Duration.ofDays(retentionDays - 1));
  ...
  void cleanupKeepsRecordsAtThreshold() {
    ...
    notificationRepository.insert(
        notificationRecord(NotificationStatus.SENT, threshold, threshold));
  ...
  private NotificationRecord notificationRecord(
    ...
    final Instant leaseUntil = status == NotificationStatus.PROCESSING ? createdAt : null;
    final String lockedBy = status == NotificationStatus.PROCESSING ? "test-worker" : null;
    final Instant nextRetryAt = status == NotificationStatus.PENDING ? createdAt : null;
    final Instant occurredAt = createdAt;
  ...
  private int count(String sql, Instant threshold) {
    ...
    final Integer result = jdbcTemplate.queryForObject(sql, params, Integer.class);
