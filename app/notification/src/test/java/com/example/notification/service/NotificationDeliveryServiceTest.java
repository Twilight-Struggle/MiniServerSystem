...
class NotificationDeliveryServiceTest {
    ...
    @Test
    void computeBackoffSecondsReturnsValueWithinRangeBeforeCap() {
        // cap 未到達時の指数バックオフ + ジッター範囲を検証する
        int attempt = 3;
        long min = minBackoffSeconds(attempt);
        long max = maxBackoffSeconds(attempt);

        long backoffSeconds = service.computeBackoffSeconds(attempt);

        // 乱数ジッターが入っても範囲内に収まることが重要
        assertThat(backoffSeconds).isBetween(min, max);
    }

    @Test
    void computeBackoffSecondsReturnsValueWithinRangeAfterCap() {
    ...
    void handleFailureSchedulesRetryWhenAttemptsRemaining() {
        ...
        long min = minBackoffSeconds(2);
        long max = maxBackoffSeconds(2);
        assertThat(nextRetryAt).isAfterOrEqualTo(FIXED_NOW.plusSeconds(min));
        assertThat(nextRetryAt).isBeforeOrEqualTo(FIXED_NOW.plusSeconds(max));
    ...
    private NotificationRecord notificationRecord(int attemptCount) {
        ...
        Instant leaseUntil = FIXED_NOW.plusSeconds(PROPERTIES.leaseSeconds());
    ...
    private long minBackoffSeconds(int attempt) {
        // ジッター下限を適用した期待レンジの最小値
        double exp = PROPERTIES.backoffBaseSeconds()
                * Math.pow(PROPERTIES.backoffExponentBase(), (attempt - 1));
        double capped = Math.min(exp, PROPERTIES.backoffMaxSeconds());
        return Math.max(PROPERTIES.backoffMinSeconds(),
                (long) Math.ceil(capped * PROPERTIES.backoffJitterMin()));
    }

    private long maxBackoffSeconds(int attempt) {
        // ジッター上限を適用した期待レンジの最大値
        double exp = PROPERTIES.backoffBaseSeconds()
                * Math.pow(PROPERTIES.backoffExponentBase(), (attempt - 1));
        double capped = Math.min(exp, PROPERTIES.backoffMaxSeconds());
        return Math.max(PROPERTIES.backoffMinSeconds(),
                (long) Math.ceil(capped * PROPERTIES.backoffJitterMax()));
    }
}
