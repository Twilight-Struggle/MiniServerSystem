...
class NotificationClaimRepositoryTest extends AbstractPostgresContainerTest {
  ...
  void claimMovesPendingToProcessingWithLock() {
    ...
    final List<NotificationRecord> claimed =
        notificationRepository.claimPendingForUpdate(
            properties.batchSize(), now, leaseUntil, "test-host");

    assertThat(claimed).hasSize(1);
    final NotificationRecord claimedRecord = claimed.get(0);
  ...
  void claimRecoversExpiredProcessing() {
    final Instant now = Instant.now();
    final Instant expiredLease = now.minus(properties.lease());
    final NotificationRecord record =
        new NotificationRecord(
            UUID.randomUUID(),
            UUID.randomUUID(),
    ...
    final List<NotificationRecord> claimed =
        notificationRepository.claimPendingForUpdate(
            properties.batchSize(), now, leaseUntil, "recovered-host");
