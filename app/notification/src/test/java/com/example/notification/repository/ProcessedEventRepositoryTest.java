...
class ProcessedEventRepositoryTest extends AbstractPostgresContainerTest {
  ...
  void insertIfAbsentReturnsFalseOnDuplicateEventId() {
    ...
    final Instant firstProcessedAt = BASE_TIME;
    final Instant secondProcessedAt = BASE_TIME.plus(THRESHOLD_GAP);

    final boolean firstInsert = processedEventRepository.insertIfAbsent(eventId, firstProcessedAt);
    final boolean secondInsert =
        processedEventRepository.insertIfAbsent(eventId, secondProcessedAt);
  ...
  void deleteOlderThanKeepsBoundaryAndNewerRows() {
    ...
    final UUID olderId = UUID.randomUUID();
    final UUID boundaryId = UUID.randomUUID();
    final UUID newerId = UUID.randomUUID();
    ...
    final int deleted = processedEventRepository.deleteOlderThan(threshold);
  ...
  private int countAll() {
    ...
    final Integer result =
        jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM processed_events", new MapSqlParameterSource(), Integer.class);
