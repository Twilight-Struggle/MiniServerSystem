...
class NotificationRepositoryTest extends AbstractPostgresContainerTest {
  ...
  void markSentClearsLockAndRetryFields() {
    final NotificationRecord record = processingRecord(BASE_TIME);
    notificationRepository.insert(record);
    final Instant sentAt = BASE_TIME.plus(THRESHOLD_GAP);

    final int updated = notificationRepository.markSent(record.notificationId(), sentAt, LOCKED_BY);

    assertThat(updated).isEqualTo(1);

    final Map<String, Object> row = fetchRow(record.notificationId());
  ...
  void markRetrySchedulesNextAttemptAndUnlocks() {
    final NotificationRecord record = processingRecord(BASE_TIME);
    notificationRepository.insert(record);
    final Instant nextRetryAt = BASE_TIME.plus(THRESHOLD_GAP);

    final int updated =
        notificationRepository.markRetry(
            record.notificationId(), RETRY_ATTEMPT_COUNT, nextRetryAt, false, LOCKED_BY);
  ...
  void markRetryMarksFailedAndClearsNextRetry() {
    ...
    final int updated =
        notificationRepository.markRetry(
            record.notificationId(), RETRY_ATTEMPT_COUNT, nextRetryAt, true, LOCKED_BY);
  ...
  void insertStoresInstantFields() {
    final Instant occurredAt = BASE_TIME.minus(THRESHOLD_GAP);
  ...
  void countStaleActiveCountsOnlyOlderPendingAndProcessing() {
    final Instant threshold = BASE_TIME;
    final Instant older = BASE_TIME.minus(THRESHOLD_GAP);
    final Instant newer = BASE_TIME.plus(THRESHOLD_GAP);

    final List<UUID> staleRecords = new ArrayList<>();
    ...
    final int count = notificationRepository.countStaleActive(threshold);
  ...
  void deleteSentOrFailedOlderThanRemovesOnlyOlderSentAndFailed() {
    ...
    final List<UUID> sentRecords = new ArrayList<>();
    final List<UUID> failedRecords = new ArrayList<>();
    final List<UUID> deleteTargets = new ArrayList<>();

    final UUID oldSent = insertRecord(NotificationStatus.SENT, older);
    final UUID oldFailed = insertRecord(NotificationStatus.FAILED, older);
    deleteTargets.add(oldSent);
    deleteTargets.add(oldFailed);
    sentRecords.add(oldSent);
    failedRecords.add(oldFailed);

    final UUID boundarySent = insertRecord(NotificationStatus.SENT, threshold);
    final UUID newerFailed = insertRecord(NotificationStatus.FAILED, newer);
    ...
    final int deleted = notificationRepository.deleteSentOrFailedOlderThan(threshold);
  ...
  private UUID insertRecord(NotificationStatus status, Instant createdAt) {
    final String lockedBy = status == NotificationStatus.PROCESSING ? LOCKED_BY : null;
    final Instant lockedAt = status == NotificationStatus.PROCESSING ? createdAt : null;
    final Instant leaseUntil =
        status == NotificationStatus.PROCESSING ? createdAt.plus(THRESHOLD_GAP) : null;
    final Instant nextRetryAt = status == NotificationStatus.PENDING ? createdAt : null;
    final Instant sentAt = status == NotificationStatus.SENT ? createdAt : null;
    final int attemptCount =
        status == NotificationStatus.PROCESSING ? INITIAL_ATTEMPT_COUNT : DEFAULT_ATTEMPT_COUNT;
  ...
  private Map<String, Object> fetchRow(UUID notificationId) {
    ...
    final MapSqlParameterSource params =
        new MapSqlParameterSource().addValue("notificationId", notificationId);
    return jdbcTemplate.queryForMap(sql, params);
  }

  private Instant getInstant(Map<String, Object> row, String column) {
    final Timestamp timestamp = (Timestamp) row.get(column);
    return timestamp == null ? null : timestamp.toInstant();
  }

  private int getInt(Map<String, Object> row, String column) {
    final Number value = (Number) row.get(column);
    return value == null ? 0 : value.intValue();
  }

  private int countByStatus(String status) {
    final MapSqlParameterSource params = new MapSqlParameterSource().addValue("status", status);
    final Integer result =
        jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM notifications WHERE status = :status", params, Integer.class);
