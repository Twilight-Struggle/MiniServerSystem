 ...
 * 何を: terminated advisory の stream_seq 保存と ack/nak を検証する
 * なぜ: 恒久的失敗の DLQ 登録動作を最低限保証するため
 */
...
class NotificationTerminatedAdvisorySubscriberTest {

  private static final String SUBJECT =
      "$JS.EVENT.ADVISORY.CONSUMER.MSG_TERMINATED.entitlement-events.notification-entitlement-consumer";
  private static final String STREAM = "notification-terminated-advisory-dlq";
  private static final String DURABLE = "notification-terminated-advisory-consumer";
  ...
  private NotificationTerminatedAdvisorySubscriber subscriber;
  ...
  void setUp() {
    final NotificationNatsProperties natsProperties =
        new NotificationNatsProperties(
            ...
            ACK_WAIT,
            MAX_DELIVER);
    final NotificationNatsTerminatedAdvisoryProperties advisoryProperties =
        new NotificationNatsTerminatedAdvisoryProperties(SUBJECT, STREAM, DURABLE);
    subscriber =
        new NotificationTerminatedAdvisorySubscriber(
            connection,
            natsProperties,
            advisoryProperties,
            dlqRepository,
