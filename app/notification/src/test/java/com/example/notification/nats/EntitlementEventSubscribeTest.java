...
class EntitlementEventSubscribeTest {
  ...
  @BeforeEach
  void setUp() {
    final NotificationNatsProperties properties =
  ...
  void termWhenHandlerFailsPermanently()
    ...
    doReturn(subscription)
        ...
        .subscribe(
            ...
            any(PushSubscribeOptions.class));
    doThrow(
            new NotificationEventPermanentException(
                "bad payload", new IllegalArgumentException("boom")))
        .when(eventHandler)
        .handleEntitlementEvent(any(EntitlementEvent.class));
  ...
  @Test
  void swallowExceptionWhenNakFailsViaJetStreamHandlerWiring()
      throws IOException, InterruptedException, JetStreamApiException {
    final Message message = mock(Message.class);
    stubConnection();
    when(message.getData()).thenReturn(buildEventBytes());
    ...
    doThrow(new DataAccessResourceFailureException("boom"))
        .when(eventHandler)
        .handleEntitlementEvent(any(EntitlementEvent.class));
    doThrow(new IllegalStateException("nak-failed")).when(message).nak();
