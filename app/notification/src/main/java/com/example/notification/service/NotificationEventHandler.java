...
public class NotificationEventHandler {
    ...
    private String serializePayload(NotificationFromEntitlementPayload payload) {
        try {
            return objectMapper.writeValueAsString(payload);
        } catch (JsonProcessingException ex) {
            // シリアライズ失敗は実装バグのため恒久的に扱う
            throw new NotificationEventPermanentException("notification payload serialization failure", ex);
        }
    }
}
