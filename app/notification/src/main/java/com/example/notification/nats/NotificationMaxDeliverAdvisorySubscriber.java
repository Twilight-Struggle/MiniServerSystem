...
public class NotificationMaxDeliverAdvisorySubscriber {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(NotificationMaxDeliverAdvisorySubscriber.class);
  ...
  public NotificationMaxDeliverAdvisorySubscriber(
  ...
  public void start() {
    ...
    try {
      ...
      logger.info(
          "notification advisory subscriber started subject={} stream={} durable={}",
          advisoryProperties.subject(),
  ...
  void handleMessage(Message message) {
    try {
      ...
      logger.warn(
          "temporary failure while handling advisory payload subject={}",
          advisoryProperties.subject(),
          ex);
      nakSilently(message);
  ...
  private void ensureStream() throws IOException, JetStreamApiException {
    ...
    logger.info(
        "notification advisory stream ensured stream={} subject={}",
        advisoryProperties.stream(),
        advisoryProperties.subject());
  }
