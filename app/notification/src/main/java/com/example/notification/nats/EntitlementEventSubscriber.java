...
public class EntitlementEventSubscriber {
  ...
  public void start() {
    ...
    try {
      ...
      logger.info(
          "notification subscriber started subject={} stream={} durable={}",
          properties.subject(),
          properties.stream(),
          properties.durable());
  ...
  void handleMessage(Message message) {
    ...
    try {
      final EntitlementEvent event = EntitlementEvent.parseFrom(message.getData());
      putMdc(mdcCloseables, "event_id", event.getEventId());
      putMdc(mdcCloseables, "user_id", event.getUserId());
      putMdc(mdcCloseables, "event_type", event.getEventType().name());
      putMdc(mdcCloseables, "source_id", event.getSourceId());
  ...
  private void ensureStream() throws IOException, JetStreamApiException {
    ...
    logger.info(
        "notification stream ensured stream={} subject={} duplicateWindow={}",
        properties.stream(),
        properties.subject(),
